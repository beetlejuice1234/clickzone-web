-- Feature — allow a 'repair' / service line item on a sale (no stock). Based on the live
-- 20260707_multi_trade_in checkout; the ONLY change is a new item-loop branch for type='repair':
-- it touches no stock and keeps the client-entered parts cost (costPrice) so owner profit is accurate.
-- Everything else (phones, accessories, trade-ins, bill numbering) is unchanged. Staging → prod.

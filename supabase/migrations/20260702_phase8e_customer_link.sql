-- Phase 8e — attach a customer at checkout (Issue: customer info). STAGING ONLY.
--
-- customers is owner-only under RLS (policy customers_owner_all = auth_role()='owner'), so staff
-- cannot SELECT/INSERT it directly. checkout() is SECURITY DEFINER (already writes owner-only
-- phone_costs), so it can create/link a customer on the staff's behalf WITHOUT granting staff any
-- read access. An optional payload.customer {name,nic,whatsapp} is deduped (NIC then WhatsApp),
-- inserted or reused, and linked via sales.customer_id. payment_method already flows through
-- unchanged (column default 'cash'). Everything else in checkout is identical to Phase 8b.
...
BEGIN
  ...
  -- Optional customer attach (owner-only table; written here via SECURITY DEFINER so staff need no read access).
  ...
  VALUES (v_sale_id, v_bill_id, payload->>'customer_whatsapp', v_items_out, v_total_rev, v_total_disc,
      ...
      v_customer_id, v_trade_val, v_total_rev - v_trade_val);

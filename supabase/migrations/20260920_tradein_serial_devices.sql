-- Migration: 20260920_tradein_serial_devices.sql
-- 1. Upgrade checkout RPC:
--    - Allow selling in-stock phones, tablets, and watches by IMEI, serial_number, or ID.
--    - Support trade-ins for tablets (iPads) and watches (Apple Watches) with serial_number and device_type.
-- 2. Upgrade find_phone_by_identifier RPC:
--    - Case-insensitive & trimmed search on serial_number and IMEI.
...
DECLARE
  ...
  v_ti_imei text; v_ti_serial text; v_ti_dev_type text;
BEGIN
  ...
  FOR v_item IN SELECT e FROM jsonb_array_elements(coalesce(payload->'items','[]'::jsonb)) AS e LOOP
    IF (v_item->>'type')='phone' THEN
      SELECT id INTO v_phone_id FROM phones
        WHERE (imei = (v_item->>'identifier')
            OR lower(btrim(serial_number)) = lower(btrim(v_item->>'identifier'))
            OR id = (v_item->>'identifier')
            OR (v_item->>'id' IS NOT NULL AND id = (v_item->>'id')))
          AND is_deleted = false AND status = 'in-stock'
        FOR UPDATE LIMIT 1;
      IF v_phone_id IS NULL THEN
        RAISE EXCEPTION 'Phone/Device % is not available (already sold or removed)', (v_item->>'identifier');
      END IF;
    ...
    ELSIF (v_item->>'type')='repair' THEN
      -- service line: no stock touched; keep the entered parts cost (feeds owner profit).
      v_items_out := v_items_out || jsonb_build_array((v_item - 'costPrice') || jsonb_build_object('costPrice', coalesce((v_item->>'costPrice')::int,0)));
    ELSE
      RAISE EXCEPTION 'Unknown item type: %', (v_item->>'type');
    END IF;
  END LOOP;

  -- Calculate total trade-in valuation
  ...
  -- Generate sequential or fallback bill ID
  ...
  -- Process trade-ins (phones, tablets, watches, accessories)
  FOR v_ti IN SELECT e FROM jsonb_array_elements(v_tis) e LOOP
    ...
    IF v_ti_type = 'accessory' THEN
      ...
      END IF;
    ELSE
      -- Device trade-in: phone, tablet, or watch
      v_ti_imei := nullif(btrim(v_ti->>'imei'),'');
      v_ti_serial := nullif(btrim(v_ti->>'serial_number'),'');
      v_ti_dev_type := coalesce(nullif(v_ti->>'device_type',''), v_ti_type, 'phone');

      IF v_ti_imei IS NOT NULL AND EXISTS (SELECT 1 FROM phones WHERE imei = v_ti_imei AND is_deleted=false AND status='in-stock') THEN
        RAISE EXCEPTION 'Trade-in IMEI % is currently active in stock', v_ti_imei;
      END IF;
      IF v_ti_serial IS NOT NULL AND EXISTS (SELECT 1 FROM phones WHERE lower(btrim(serial_number)) = lower(btrim(v_ti_serial)) AND is_deleted=false AND status='in-stock') THEN
        RAISE EXCEPTION 'Trade-in Serial % is currently active in stock', v_ti_serial;
      END IF;

      -- Check if prior sold/deleted unit exists to revive
      v_ingested_id := NULL;
      IF v_ti_imei IS NOT NULL THEN
        SELECT id INTO v_ingested_id FROM phones
          WHERE imei = v_ti_imei AND (is_deleted = true OR status <> 'in-stock')
          ORDER BY local_updated_at DESC NULLS LAST LIMIT 1 FOR UPDATE;
      ELSIF v_ti_serial IS NOT NULL THEN
        SELECT id INTO v_ingested_id FROM phones
          WHERE lower(btrim(serial_number)) = lower(btrim(v_ti_serial)) AND (is_deleted = true OR status <> 'in-stock')
          ORDER BY local_updated_at DESC NULLS LAST LIMIT 1 FOR UPDATE;
      END IF;
      ...
      IF v_ingested_id IS NOT NULL THEN
        ...
        UPDATE phones SET status='in-stock', is_deleted=false, source='trade-in',
          ...
          device_type = v_ti_dev_type,
          serial_number = coalesce(v_ti_serial, serial_number),
        ...
        IF v_old_cost IS DISTINCT FROM round(v_item_val)::int THEN
          ...
          VALUES (v_uid,'phone_costs.update',v_ingested_id,
            jsonb_build_object('reason','checkout trade-in revive','old_cost',v_old_cost,'new_cost',round(v_item_val)::int,'imei',v_ti_imei,'serial',v_ti_serial));
        END IF;
        v_revived := true;
      ELSE
        v_ingested_id := 'p_'||replace(gen_random_uuid()::text,'-','');
        INSERT INTO phones (id,imei,serial_number,model,storage,color,condition,battery_health,icloud_status,target_sale_price,
            status,date_added,is_deleted,local_updated_at,source,device_type,store_id)
        VALUES (v_ingested_id, v_ti_imei, v_ti_serial, coalesce(v_ti->>'model','Trade-in'), coalesce(v_ti->>'storage',''),
            ...
            to_char((now() AT TIME ZONE 'Asia/Colombo'),'YYYY-MM-DD'), false, now()::text, 'trade-in', v_ti_dev_type, v_store);
        INSERT INTO phone_costs (phone_id,cost_price) VALUES (v_ingested_id, round(v_item_val)::int)
          ON CONFLICT (phone_id) DO UPDATE SET cost_price=EXCLUDED.cost_price;
      END IF;
    END IF;
    ...
    ELSE
      ...
      VALUES (v_exchange_id, v_sale_id, 'trade-in', coalesce(v_ti_imei, v_ti_serial, 'NONE'), v_ti->>'model', v_item_val, v_ti->>'condition',
...
-- Update find_phone_by_identifier with case-insensitive serial lookup
CREATE OR REPLACE FUNCTION public.find_phone_by_identifier(identifier text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_row phones%rowtype; v_id text := btrim(identifier);
BEGIN
  IF public.auth_role() <> 'owner' THEN RAISE EXCEPTION 'find_phone_by_identifier: owner only'; END IF;
  IF v_id IS NULL OR v_id = '' THEN RETURN jsonb_build_object('found', false); END IF;

  SELECT * INTO v_row FROM phones
    WHERE (imei = v_id OR lower(btrim(serial_number)) = lower(v_id)) AND is_deleted = false AND status = 'in-stock'
    LIMIT 1;
  ...
  SELECT * INTO v_row FROM phones
    WHERE (imei = v_id OR lower(btrim(serial_number)) = lower(v_id)) AND (is_deleted = true OR status <> 'in-stock')

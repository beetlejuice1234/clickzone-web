-- Client polish — prefix the sequential invoice number with "INV-" so bills read "INV-1050"
-- (the format the owner picked) instead of a bare "1050". This is the 20260703_sequential_bill_no
-- checkout() verbatim with ONE change: v_bill_id := 'INV-'||v_seq::text (was v_seq::text).
-- Legacy CZ-YYMMDD-HEX id is still used when next_bill_no is NULL. Staging first, then production.
...
BEGIN
  ...
  -- INV- prefix so the bill reads "INV-1050" (owner-chosen format).

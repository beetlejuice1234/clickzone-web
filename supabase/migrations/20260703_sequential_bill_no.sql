-- Client fix — sequential invoice numbers.
-- Owner sets a per-store starting number (stores.next_bill_no); checkout consumes it and
-- increments atomically inside the sale txn (so a rolled-back checkout leaves no gap). When
-- next_bill_no is NULL the legacy CZ-YYMMDD-HEX id is used. Staging first, then production.

ALTER TABLE public.stores ADD COLUMN IF NOT EXISTS next_bill_no bigint;

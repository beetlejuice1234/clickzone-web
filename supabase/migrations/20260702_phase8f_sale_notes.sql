-- Phase 8f — persist a per-sale "Special Notes" note at checkout. STAGING ONLY.
--
-- The bill needs an optional free-text note that also survives on Sales Log reprints. The sales table
-- already has an unused `notes text` column (exposed by v_sales_public and v_sales_profit), so no
-- schema/view change is needed — only checkout() must write it. This is an exact copy of the Phase 8e
-- checkout() with two additions: it reads payload->>'notes' (empty => NULL) and inserts it into
-- sales.notes. Everything else is identical to Phase 8e. Replayable at cutover.
...
DECLARE
  ...
  v_notes text := nullif(payload->>'notes','');   -- Phase 8f: per-sale special note
BEGIN

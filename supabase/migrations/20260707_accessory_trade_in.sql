-- Client fix — allow trading in an ACCESSORY as part-payment (not just phones).
-- Based on the live 20260703_sequential_bill_no checkout (bare-number invoice ids — the owner chose
-- plain digits, so this also supersedes/reverts the short-lived INV- prefix experiment).
-- New: payload.trade_in.type = 'accessory' → add the traded accessory to resale stock at the
-- valuation (as its cost), reduce net_payable by the valuation, and record an 'accessory-trade-in'
-- exchange row (so it still counts in the trade-in tally). 'phone' (default) is unchanged.
-- Staging first, then production (with explicit approval).
...
DECLARE
  ...
  v_ti_type text := 'phone'; v_acc_sku text; v_ti_qty int;
...
BEGIN
  ...
  IF v_ti IS NOT NULL AND v_ti <> 'null'::jsonb THEN
    ...
    IF v_ti_type = 'accessory' THEN
      -- Accessory trade-in: add it to resale stock at the valuation (as cost); no phone touched.
      ...
      ELSE
        ...
        INSERT INTO accessories (sku,name,quantity,cost_price,sale_price,store_id,is_deleted,local_updated_at)
          VALUES (v_acc_sku, coalesce(nullif(v_ti->>'model',''),'Trade-in accessory'), v_ti_qty,
              round(v_trade_val/v_ti_qty)::int, 0, v_store, false, now()::text);
      END IF;

    ELSE
      -- Phone trade-in (unchanged): revive a prior unit or create a new in-stock phone.
  ...
  -- Bare digits (owner chose plain numbers, no prefix).
  ...
  IF v_ti IS NOT NULL AND v_ti <> 'null'::jsonb THEN
    ...
    IF v_ti_type = 'accessory' THEN
      ...
      VALUES (v_exchange_id, v_sale_id, 'accessory-trade-in', v_acc_sku, coalesce(v_ti->>'model','Accessory'),
          v_trade_val, v_ti->>'condition', v_ti->>'notes', null, v_ti->>'customer_name', v_ti->>'customer_nic',
  ...
  RETURN jsonb_build_object('sale_id',v_sale_id,'bill_id',v_bill_id,'ingested_phone_id',v_ingested_id,
      ...
      'accessory_trade_in_sku', CASE WHEN v_ti_type='accessory' THEN v_acc_sku ELSE null END);
END $function$;
